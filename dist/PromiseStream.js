...
class PromiseStream {
  ...
  pipe(callback) {
    //.then everything, not waiting for previous, not passing val of prev cb
    this.promises = this.promises.map((promise, index, source) => promise.then(value => callback(value, index, source)));
    return this;
  }

  pipeOrdered(callback) {
    //.then everything, waiting for previous, not passing val of prev cb
  ...
  all() {
    ...
    this.promises = this.promises.map((promise, index, source) => promise.catch(async error => await callback(error, index, source)));
    return this;
  }
